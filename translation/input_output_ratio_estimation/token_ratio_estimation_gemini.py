...
# - Set to -1 for no thinking limitation (unlimited)
# - Set to positive number for specific thinking budget (e.g., 10000)
THINKING_BUDGET = -1

# Input and output files (these are in the parent directory)
INPUT_JSON_FILE = "../Cujo - Stephen King_segmented.json"
OUTPUT_CSV_FILE = "gemini_token_ratio_thinking.csv"
...
def main():
    """Main function to estimate token ratio for Gemini models."""
    ...
    if THINKING_BUDGET == 0:
        print("Thinking budget: 0 (thinking disabled)")
    elif THINKING_BUDGET == -1:
        print("Thinking budget: -1 (unlimited thinking)")
    else:
    ...
    # Prepare CSV file
    csv_data = []
    csv_data.append(['input_tokens', 'output_tokens', 'thinking_tokens'])
    ...
    for batch_num, num_paragraphs in enumerate(paragraph_counts, start=1):
        ...
        if result.get('success'):
            ...
            thinking_tokens = actual_tokens.get('thoughts_tokens', 0)
            total_tokens = actual_tokens.get('total_tokens', 0)
            
            print(f"\nToken counts:")
            print(f"  Input tokens (prompt):     {input_tokens:,}")
            print(f"  Output tokens (candidates): {output_tokens:,}")
            print(f"  Thinking tokens (thoughts): {thinking_tokens:,}")
    ...
    # Calculate and display average ratio and statistics
    if len(csv_data) > 1:
        total_input = sum(row[0] for row in csv_data[1:])
        total_output = sum(row[1] for row in csv_data[1:])
        total_thinking = sum(row[2] for row in csv_data[1:])
        ...
        if total_input > 0:
            ...
            print(f"  Grand total tokens:    {total_input + total_output + total_thinking:,}")
