...
class TextToPDFConverter:
    ...
    def __init__(self, translated_text_folder="translation/translated_text", pdf_output_folder="translation/pdf_output"):
        """
    ...
    def get_first_translated_file(self, file_name="default"):
        """
        Get the file from the translated_text folder.
        If file_name is "default", returns the first file alphabetically.
        Otherwise, returns the specified file.
        
        Args:
            file_name (str): Filename to pick, or "default" for first file
        
        Returns:
            str: Path to the translated text file
        """
        ...
        if file_name.lower() == "default":
            ...
            file_path = os.path.join(self.translated_text_folder, file_name)
            if not os.path.exists(file_path):
                raise FileNotFoundError(f"File '{file_name}' not found in '{self.translated_text_folder}'")
            return file_path
    ...
    def create_pdf(self, file_name="default"):
        """
        Create PDF from the translated text file.
        ...
        try:
            ...
            file_path = self.get_first_translated_file(file_name)
...
def main():
    ...
    try:
        # Create converter instance with INPUT_PATH and OUTPUT_PATH constants
        converter = TextToPDFConverter(translated_text_folder=INPUT_PATH, pdf_output_folder=OUTPUT_PATH)
        
        # Convert text to PDF using FILE_NAME constant
        pdf_path = converter.create_pdf(file_name=FILE_NAME)
